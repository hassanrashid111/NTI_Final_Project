...
def run_ai_forecast_pipeline(df_raw, service_level_z=1.65, lead_time_days=7, review_period_days=7):
    ...
    if model is not None:
        try:
            ...
            y_pred_log = model.predict(X_mat)
    ...
    df["sku_daily_avg"] = np.maximum(0.5, df["sku_daily_avg"])
    df["safety_stock"] = np.ceil(service_level_z * df["sku_daily_std"] * np.sqrt(lead_time_days)).astype(int)
    df["reorder_point"] = np.ceil((df["sku_daily_avg"] * lead_time_days) + df["safety_stock"]).astype(int)
    df["target_stock_level"] = np.ceil(df["reorder_point"] + (df["sku_daily_avg"] * review_period_days)).astype(int)
    df["recommended_order_qty"] = np.maximum(0, df["target_stock_level"] - df["current_stock"]).astype(int)
