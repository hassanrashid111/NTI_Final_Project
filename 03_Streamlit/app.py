...
# Try importing project config & utils
try:
    import config, utils
except ImportError:
...
# ══════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        if st.button("⚡ Load Grocery Sample", use_container_width=True):
            ...
            if fpath.exists():
                st.session_state["active_df"] = pd.read_csv(fpath)
                st.success(f"✅ Loaded Grocery Sample ({len(st.session_state['active_df']):,} rows)! Dashboard updated.")
    ...
    with c2:
        ...
        if st.button("⚡ Load Beverages Sample", use_container_width=True):
            ...
            if fpath.exists():
                st.session_state["active_df"] = pd.read_csv(fpath)
                st.success(f"✅ Loaded Beverages Sample ({len(st.session_state['active_df']):,} rows)! Dashboard updated.")
    ...
    with c3:
        ...
        if st.button("⚡ Load Store #1 Sample", use_container_width=True):
            ...
            if fpath.exists():
                st.session_state["active_df"] = pd.read_csv(fpath)
                st.success(f"✅ Loaded Store #1 Flagship ({len(st.session_state['active_df']):,} rows)! Dashboard updated.")
    ...
    with c4:
        ...
        if st.button("⚡ Load Store #44 Sample", use_container_width=True):
            ...
            if fpath.exists():
                st.session_state["active_df"] = pd.read_csv(fpath)
                st.success(f"✅ Loaded Store #44 Hypermarket ({len(st.session_state['active_df']):,} rows)! Dashboard updated.")
    ...
    if uploaded_file is not None:
        try:
            ...
            st.session_state["active_df"] = df_up
            st.success(f"✅ Uploaded `{uploaded_file.name}` ({len(df_up):,} rows)! Schema verified. Dynamic predictions recalculated.")
            st.dataframe(df_up.head(10), use_container_width=True)
        except Exception as e:
            st.error(f"Error parsing CSV: {e}")

    if "active_df" in st.session_state:
        st.info(f"📊 **Active Selected Dataset:** {len(st.session_state['active_df']):,} total rows loaded.")
